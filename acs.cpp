	...
	while(pass->session->isActive()) {
		...
		if (strchr("SDGENFLAMIQ", toupper(ch))) {		
		    ...
		    switch(toupper(ch)) {
			    ...
			    case 'D': mod_dotw(&acs->dotw);	    break;
        		case 'E': mod_Sflag(&acs->eflags);	break;
        		case 'F': mod_Sflag(&acs->fflags);	break;
        		case 'G':
        			acs->gender++;
		         	if(acs->gender>2)	acs->gender=0;
				    break;

        		case 'I':	mod_Lflag(&acs->fgroup); break;
        		case 'L':
                ...
                case 'M': mod_Lflag(&acs->mgroup); break;
            	case 'N':
